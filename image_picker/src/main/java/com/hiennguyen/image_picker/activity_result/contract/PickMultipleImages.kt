...
class PickMultipleImages : ActivityResultContract<ImagePickerConfig, List<Image>?>() {
    ...
    override fun createIntent(context: Context, input: ImagePickerConfig): Intent {
        return Intent(context, ImagePickerActivity::class.java)
            .putExtra(Constants.IMAGE_PICKER_TYPE, ImagePickerType.MULTIPLE_IMAGES as Parcelable)
            .putExtra(Constants.IMAGE_PICKER_CONFIG, input)
    }

    override fun parseResult(resultCode: Int, intent: Intent?): List<Image>? {
        return if (intent == null || resultCode != Activity.RESULT_OK) null else intent.getParcelableArrayListExtra(
            Constants.MULTIPLE_IMAGES_RESULT)
    }
}